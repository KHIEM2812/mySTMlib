...
void USARTx_DMA_TX_IRQHandler(void) {
	if (DMA_GetITStatus(USARTx_TX_DMA_STREAM, USARTx_TX_DMA_IT_TCIF) != RESET) { // ensure check interrupt flag for receive data
		USARTx_TX_DMA_STREAM->CR &= ~(uint32_t) DMA_SxCR_EN;
		USARTx->CR3 &= (uint16_t) ~USART_DMAReq_Tx;
		DMA_ClearITPendingBit(USARTx_TX_DMA_STREAM, USARTx_TX_DMA_IT_TCIF); /// Clear Interrupt pending bit
	} else if (DMA_GetITStatus(USARTx_TX_DMA_STREAM, USARTx_TX_DMA_IT_TEIF) != RESET) {
		DMA_ClearITPendingBit(USARTx_TX_DMA_STREAM, USARTx_TX_DMA_IT_TEIF);
	} else if (DMA_GetITStatus(USARTx_TX_DMA_STREAM, USARTx_TX_DMA_IT_FEIF) != RESET) {
		DMA_ClearITPendingBit(USARTx_TX_DMA_STREAM, USARTx_TX_DMA_IT_FEIF);
	}
}
